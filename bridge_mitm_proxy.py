...
# Rithmic translator (loaded on demand for RITHMIC_MODE)
_rithmic_translator = None
def _get_translator():
    global _rithmic_translator
    if _rithmic_translator is None and RITHMIC_MODE:
        try:
            from rithmic_translator import RithmicTranslator
            _rithmic_translator = RithmicTranslator(
                os.path.join(os.path.dirname(__file__), "cqg_test")
            )
            log.info("[RTHM] RithmicTranslator loaded")
        except Exception as e:
            log.error(f"[RTHM] Failed to load translator: {e}")
    return _rithmic_translator
...
RITHMIC_MODE = os.environ.get("RITHMIC_MODE", "").lower() in ("1", "true", "yes")
...
# Track T&S batch count per request_id to inject force-complete signals
_ts_batch_counts = {}
_ts_pending_completions = set()
_ts_completed_ids = set()
...
async def forward_client_to_cqg(client_r, cqg_w, initial_remaining=b"", http_done=False, is_historical=False):
    buf = FrameBuffer()
    if initial_remaining:
        buf.feed(initial_remaining)
    try:
        while True:
            # Process WebSocket frames first (critical if initial_remaining has data)
            while True:
...
# ─── Thread pool for CPU-bound Protobuf parsing/serializing ─────────────────────
import concurrent.futures
from asyncio import get_running_loop

# We keep a global thread pool executor for running CPU-bound Protobuf operations
# to keep the asyncio event loop completely free and responsive to PING/PONG and socket buffers.
_executor = concurrent.futures.ThreadPoolExecutor(max_workers=config.BRIDGE_THREAD_POOL_WORKERS)
...
def process_and_patch_server_msg(payload: bytes, fin: int, opcode: int):
    ...
    Parses the message exactly once, logs it, and patches it if needed.
    """
    if not PROTOBUF_AVAILABLE:
        return None
    try:
        ...
        # 1. Log the server message
        log_server_msg_parsed(msg)
        
        # 2. Patch the server message
        global _ts_batch_counts, _ts_pending_completions, _ts_completed_ids
        patched = False
        for tsr in msg.time_and_sales_reports:
            if tsr.result_code == 0 and len(tsr.quotes) > 0:
                rid = tsr.request_id
                if rid not in _ts_completed_ids and not tsr.is_report_complete:
                    _ts_batch_counts[rid] = _ts_batch_counts.get(rid, 0) + 1
                    if _ts_batch_counts[rid] >= 15 and rid not in _ts_pending_completions:
                        _ts_pending_completions.add(rid)
                        log.info(f"  [PATCH] T&S force-complete queued for request_id={rid} "
                                 f"(batch #{_ts_batch_counts[rid]})")
...
def log_server_msg_parsed(msg: ServerMsg):
    ...
    try:
        ...
        for rtd in msg.real_time_market_data:
            mv_detail = ""
            if rtd.market_values:
                mv = rtd.market_values[0]
                mv_detail = (f" mv[0]: day_idx={mv.day_index} "
                             f"tv={mv.total_volume} "
                             f"last={mv.scaled_last_price_no_settlement} "
                             f"close={mv.scaled_close_price}")
            log.debug(f"  [S->C] REAL_TIME_DATA: contract_id={rtd.contract_id} "
                      f"snapshot={rtd.is_snapshot} quotes={len(rtd.quotes)}"
                      f"{mv_detail}")
...
# ─── Rithmic Mode Handler ─────────────────────────────────────────────────────
async def handle_rithmic(client_r, client_w, handshake=b"", remaining=b""):
    translator = _get_translator()
    if not translator:
        log.error("[RTHM] Translator not available — closing")
        client_w.close()
        return

    await translator.connect_rithmic()
    if not translator.connected:
        log.error("[RTHM] Could not connect to Rithmic — closing")
        client_w.close()
        return

    peer = client_w.get_extra_info("peername")
    log.info(f"[RTHM] Handling {peer} via Rithmic translator")

    # Write WS upgrade response (101 Switching Protocols)
    if handshake:
        import hashlib, base64
        buf = FrameBuffer()
        buf.feed(handshake)
        ws_key = ""
        for line in handshake.decode(errors="replace").split("\r\n"):
            if line.lower().startswith("sec-websocket-key:"):
                ws_key = line.split(":", 1)[1].strip()
                break
        if ws_key:
            accept = base64.b64encode(
                hashlib.sha1((ws_key + "258EAFA5-E914-47DA-95CA-5AB9F63DC245").encode()).digest()
            ).decode()
            upgrade = (
            ...
            client_w.write(upgrade)
            await client_w.drain()
            log.info("[RTHM] WS upgrade response sent")

    client_buf = FrameBuffer()
    if remaining:
        client_buf.feed(remaining)

    async def send_responses():
        async for msg_type, payload in translator.consume_tick_queue():
            if msg_type == "server_msg":
                frame = build_ws_frame(2, payload.SerializeToString(), fin=1, mask=False)
                try:
    ...
    async def read_client():
        while True:
            frame = client_buf.extract_frame()
            if frame:
                opcode, masked, mask_key, payload, raw_frame, fin = frame
                if opcode == 8:
                    log.info("[RTHM] Client close frame")
                    return
                if opcode in (9, 10):
                    continue
                if opcode == 2:
                    response = await translator.handle_message(payload)
                    for rtype, rpayload in response:
                        if rtype == "close":
                            return
                        if rtype == "server_msg":
                            frame = build_ws_frame(2, rpayload.SerializeToString(), fin=1, mask=False)
                            client_w.write(frame)
                            await client_w.drain()
            else:
                chunk = await client_r.read(65536)
                if not chunk:
                    return
                client_buf.feed(chunk)

    tick_task = asyncio.create_task(send_responses())
    try:
        await read_client()
    finally:
        tick_task.cancel()
        try:
            ...
            pass
        await translator.disconnect_rithmic()
        log.info(f"[-] RTHM Disconnected {peer}")


# ─── Connection handler ─────────────────────────────────────────────────────────
async def handle(client_r, client_w):
    global _ts_batch_counts, _ts_pending_completions, _ts_completed_ids
    _ts_batch_counts.clear()
    _ts_pending_completions.clear()
    _ts_completed_ids.clear()
    ...
    if sni and ("historical" in sni or "deepcharts" in sni):
        is_historical = True
    elif http_done and (path == "/" or "443" not in path):
        is_historical = True

    if RITHMIC_MODE and not is_historical:
        log.info("[RTHM] Using Rithmic translator for CQG connection")
        await handle_rithmic(client_r, client_w, handshake, remaining)
    ...
    t1 = asyncio.create_task(forward_client_to_cqg(client_r, cqg_w, initial_remaining=remaining, http_done=http_done, is_historical=is_historical))
...
async def main():
    ...
    if RITHMIC_MODE:
        log.info(f"[*] MODE: Rithmic Translator (RITHMIC_MODE=1)")
    else:
